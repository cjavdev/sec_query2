module SecQuery
  class Transaction
    ...
    attr_accessor :symbol, :filing_number, :code, :date, :reporting_owner,
      :form, :type, :modes, :shares, :price, :owned, :number, :owner_cik,
      :security_name, :deemed, :exercise, :nature, :derivative, :underlying_1,
      :exercised,	:underlying_2, :expires, :underlying_3
    ...
    def self.find(entity, start, count, limit)
      ...
      node = doc.search("//td[@width='40%']")[0]
      if !node.nil?
        trans = node.parent.parent.search("//tr")
        i= start;
        query_more = false;
        for tran in trans
          td = tran.search("//td")
